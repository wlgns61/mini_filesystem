...
void print_working_directory(treeptr exit)
{//현재 위치에서 exit까지 리스트로 연결 후 역순으로 출력 후 리스트 삭제(경로의 길이가 길어질 수 있음으로 동적할당 사용)
    treeptr t=current_node;
    pathptr newnode, head, temp;
    newnode = (pathptr)malloc(sizeof(TreeNode));

    strcpy(newnode->name,t->name);
    newnode->id=t->id;
    newnode->lsibling=newnode->rsibling=NULL;
    head=newnode;
    while(t->parent!=NULL||t->lsibling!=NULL)
    {
        if(exit==t)
            break;
        if(t->parent==NULL && t->lsibling!=NULL)
            t=t->lsibling;
        else //parent!=NULL
        {
            t=t->parent;
    ...
    while(head!=NULL)
    {
        printf("%s",head->name); //출력
        if(strcmp(head->name,"/")!=0&&head->rsibling!=NULL)
            printf("/");
        head=head->rsibling;
        free(temp); //삭제
        temp=head;
...
int instruct()
{
    ...
    while(1)
    {
        ...
        {
            print_working_directory(user);
            printf("$ ");
        }
        if(current_node==root)
        {
            print_working_directory(root);
            printf("$ ");
        }
        ...
        if(current_node!=home&&current_node!=root&&current_node!=user)
        {
            printf("~/");
            print_working_directory(user);
            printf("$ ");
        ...
        else if(strcmp(inst,"mkdir")==0)
        {
            ...
            if(count> 1) //multi processing 미구현, 실제 리눅스와 실행 동작은 같음
            {
               char *p_ch;
               char dname[30];
               p_ch=strtok(path," ");
